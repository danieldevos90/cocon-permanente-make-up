...
import { withTenant, getContextConfig } from './tenant-context.js';
...
function getStages(cfg) {
  return {
    aftercare: {
      name: 'Aftercare',
      daysAfter: cfg.messageTiming.aftercare,
      tag: 'whatsapp-aftercare-sent',
    },
    browsRefresh: {
      name: 'Brows refresh',
      daysAfter: cfg.messageTiming.browsRefresh,
      tag: 'whatsapp-brows-refresh-sent',
    },
    lipsRefresh: {
      name: 'Lips refresh',
      daysAfter: cfg.messageTiming.lipsRefresh,
      tag: 'whatsapp-lips-refresh-sent',
    },
  };
}

export const whatsappStages = getStages(defaultConfig);
...
export async function sendWhatsAppForStage(input) {
  ...
  return withTenant(clientId, () => sendWhatsAppForStageInner(input, clientId));
}

async function sendWhatsAppForStageInner(input, clientId) {
  const cfg = getContextConfig() || defaultConfig;
  ...
  // Live sends require Meta-approved templates; dry-run allows pending for demos/review.
  if (template.metaStatus !== 'approved' && !cfg.dryRun) {
  ...
  if (!skipDedupe && (await hasSent({ stage, treatmentType, email, clientId }))) {
  ...
  const sendResult = await sendTemplate(
    {
      ...
      components,
      context: { stage, treatmentType, email, clientId },
    },
    { config: cfg },
  );
  ...
  if (sendResult.success && !sendResult.dryRun) {
    await markSent({ stage, treatmentType, email, clientId });
  }
 ...
 */
export function planStagesForTreatment({ treatmentType, treatmentDate = new Date(), clientId }) {
  const cfg = getContextConfig() || defaultConfig;
  const stages = getStages(cfg);
  const planned = [];
  ...
  if (stages.aftercare && cfg.treatmentTypes.includes(treatmentType)) {
    planned.push({
      stage: 'aftercare',
      treatmentType,
      scheduledDate: addDays(date, stages.aftercare.daysAfter),
      clientId: clientId || cfg.client?.id,
    });
  }
  if (treatmentType === 'wenkbrauwen' && stages.browsRefresh) {
    planned.push({
      stage: 'browsRefresh',
      treatmentType,
      scheduledDate: addDays(date, stages.browsRefresh.daysAfter),
      clientId: clientId || cfg.client?.id,
    });
  }
  if (treatmentType === 'lippen' && stages.lipsRefresh) {
    planned.push({
      stage: 'lipsRefresh',
      treatmentType,
      scheduledDate: addDays(date, stages.lipsRefresh.daysAfter),
      clientId: clientId || cfg.client?.id,
    });
  }
  return planned;
